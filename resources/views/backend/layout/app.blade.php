...
<html lang="en">
	...
	<head>
		...
		<style>
			...
			.drawer-modal .modal-content form {
				display: flex;
				flex-direction: column;
				height: 100%;
			}
...
<!--end::Post-->
		...
		<!-- Hidden session data for AJAX parsing -->
		...
		<script>
			...
			// Global Form Spinner & AJAX Handler for Modals
			document.addEventListener('DOMContentLoaded', function () {
				document.querySelectorAll('.drawer-modal form, .modal form').forEach(form => {
					if (form.classList.contains('custom-ajax')) return;
					...
					form.addEventListener('submit', function(e) {
						...
						if(btn) {
							btn.innerHTML = '<span class="spinner-border spinner-border-sm align-middle me-2"></span> Memproses...';
						...
						.then(async response => {
							if (!response.ok) {
								if (response.status === 422) {
									...
									throw {type: 'validation', html: errorHtml};
								}
								throw new Error('Terjadi kesalahan pada server (Error ' + response.status + ')');
							}
							
							// If response is OK, it might be HTML (redirected) or JSON
							let contentType = response.headers.get("content-type");
							if (contentType && contentType.indexOf("application/json") !== -1) {
								let data = await response.json();
								Swal.fire("Berhasil", data.message || "Data berhasil disimpan!", "success").then(() => window.location.reload());
							} else {
								// Parse HTML to find session flash
								let html = await response.text();
								let parser = new DOMParser();
								let doc = parser.parseFromString(html, 'text/html');
								
								let successEl = doc.querySelector('#session-success');
								let errorEl = doc.querySelector('#session-error');
								
								let errorMsg = errorEl ? errorEl.getAttribute('data-message') : '';
								let successMsg = successEl ? successEl.getAttribute('data-message') : '';
						...
						.catch(error => {
							...
							Swal.fire({
								html: error.type === 'validation' ? error.html : (error.message || "Terjadi kesalahan sistem"),
								icon: "error",
								buttonsStyling: false,
								confirmButtonText: "Ok, Perbaiki",
								customClass: { confirmButton: "btn btn-danger" }
